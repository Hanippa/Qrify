...
const QrScanner = qrScannerModule.default;
  ...
  let dark_mode_toggle = document.getElementsByClassName('dark-mode-toggle-container')[0];
...
//function 13 - hides or unhides the qr options menu

const hideQrOptions = (state) => {
  let qrOptions = document.getElementsByClassName("qr-options-container")[0];
  if(state){
...
//function 14 - save theme mode to synced storage.
    ...
    if (items) {// Check if files are present in the clipboard data
      for (let i = 0; i < items.length; i++) {
        ...
        if (item.kind === 'file' && item.type.startsWith('image/')) {
          ...
          const input_index = tab.dataset.index;
          const file_input = document.getElementById(`image-input-${input_index}`);
...
for (const element of custom_file_inputs) {
  ...
  const file_input = document.getElementById(`image-input-${input_index}`);
  let dragCounter = 0; // prevents flickering when entering/leaving child elements
